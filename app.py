...
def scp_hpb(area_cd:str, page_num:str, cat:str) -> list:
    ...
    for i in range(1, page_num + 1):        
        ...
        soup = BeautifulSoup(res.text, "html.parser")
        url_l = [
            soup.find_all("li", class_="searchListCassette")[i]
            .find("h3", class_="slcHead")
            .a.get("href")
            .split("?")[0]
            + "tel/"
            for i in range(len(soup.find_all("li", class_="searchListCassette")))
        ]
                ...
                tags = str(
                    [
                        ...
                        for e in soup.find_all(
                            ...
                            and value.startswith("fl iS offL kr"),
                        )
                        if e.text != "ジャンル"
                    ]
                )
...
if hp == "ビューティー":
    ...
    area_cd = area2cd[area]
    res = requests.get(
        f"https://beauty.hotpepper.jp/{genre_cd}/svcSA/{area_cd}/salon/PN1.html"
    )
    soup = BeautifulSoup(res.text, "html.parser")
    store_num = soup.find("span", class_="numberOfResult").text
    max_page = (
        soup.find("p", class_="pa bottom0 right0")
        .text.strip()
        .rstrip("次へ")
        .strip()
        .rstrip("ページ")
